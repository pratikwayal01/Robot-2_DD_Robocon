/*
  ...
  Author: Pratik Wayal
  Date: 2024-07-05
  Description: This Arduino sketch is writen for Robot-2 (Fully Automatic)
...
unsigned long Checkduration = 150;
...
void loop() {
  ...
  if (mod == 0) {
    ...
    if (orientation_setpoint == 90 && turnFlag == false) {
      ...
      backward(shield_val);
      if ((z_distance > 160 && z_distance < 230) && (x_distance < 700 && x_distance > 250) ) {
    ...
    if (TFlag == false && orientation_setpoint == 90) {
      ...
      Back_Shield_correct(-shield_val, output);
      forward(val);
      if (y_distance < 130 && y_distance > 0) {
        UFlag = true;
      }
    }
    ...
    if (IFlag == true && SFlag == false) {
      Back_Shield_correct(-turn_val, output);
      forward(turn_val);
